...
<img src="neural_diagram.svg" width="300">
...
> [!IMPORTANT]
> Какие рукописи нужны?
> В обучающую выборку должны быть включены одобренные рецензентами рукописи по тематике журнала. Из выборки исключаются не научные рукописи: персоналии, исторические, редакторские заметки и прочее.
> В обучающую выборку не включаются отклоненные рукописи которые 
> - отклонены, так как не соответствовали тематике журнала;
> - отклонены из-за малого количества уникального текста (например по системе "Антиплагиат");
> - отклоненные несколько раз и поданные авторами снова (рекомендуется брать только первый вариант отклоненной рукописи);
> - отозванные авторами до начала процесса рецензирования.
...
A: Это выходной сигнал (*logit*) полносвязной нейронной сети после функции активации. Формально — вероятность принадлежности к классу «отклонена». Однако эта вероятность сильно зависит от распределения классов в ваших обучающих данных (если в обучении было 52% принятых и 48% отклонённых, то 0.231 — это реально низкий шанс). Модель не возвращает абсолютную метрику качества рукописи, только относительное сходство с примерами из прошлого.
...
#### Q: Если я отклоню рукопись, опираясь на рекомендацию модели, а автор подаст жалобу или обратится в суд — что служит документальным основанием решения редакции?
A: Система не логирует, когда и какие рекомендации были выданы, что делает аудит невозможным. Система выдает рекомендации, решение остается за редактором. Я как разработчик против того, чтобы решение системы было окончательным и использовалось для диалога с автором об отказе в публикации. Отказ подтвержадается экспертом - рецензетом и никак иначе.

#### Q: Может ли модель «застрять» на устаревших представлениях журнала о публикуемой теме, если несколько лет назад редакция изменила тематику или стратегию?
A: Исторические данные кодируют прошлую редполитику, а не текущую.

#### Q: Если два рецензента, назначенных системой, уже рецензировали рукопись того же автора в прошлом году, — как система это обнаружит?
A: Ответ: никак. Система не знает, кто автор.

#### Q: Насколько устойчива система к намеренному «накачиванию» — если автор вставит в текст рукописи словарный мусор из наиболее часто встречающихся слов принятых статей?
A: Такой риск действительно существует, и именно поэтому финальную точку в судьбе рукописи ставит эксперт предметной области. Случаи, что алгоритм пропускает с очень высокой вероятностью пустую с научной точки зрения рукопись при тестировании были. 

#### Q: Если редакция переходит на эту систему, нужно ли уведомлять авторов о том, что их рукопись обрабатывается алгоритмически? Есть ли у авторов право отказаться?
A: Авторам при запросе можно рассказать в какие конкретно системы загружается рукопись до, во время и после публикации. Обычно стандартная проверка заключается в подтверждении отсутствии сведений, составляющих государственную тайну, отсутствие или корректный процент заимствований, передача мета-данных статей в наукометрические базы. При реализации системы в редакции новая статья сравниваться со всеми прошлыми рукописями отклоненными и принятыми в журнале. Если авторы захотят узнать эту информация не вижу никаких препятствий, чтобы её раскрыть. Тем более, что информация о рукописи не уходит ни в какие сторонние базы данных или сервисы и может проверяться локально.

#### Q: Что происходит с точностью, если тематика журнала узкоспециальная и большинство отклонённых рукописей отклоняются по формальным причинам (объём, оформление), а не содержательным?
A: Модель обучается на паттернах содержания, но причина отклонения может быть несодержательной. Кроме того, обычно по процессу формальная причина идёт до этапа рецензирования(предрецензирования) и на рецензирования уже попадает рукопись, прошедшая формальный этап отсева.

#### Q: Как редакция должна реагировать, если система регулярно рекомендует одних и тех же 3–4 рецензентов, создавая для них неравномерную нагрузку?
A: Система не учитывает «загрузку» рецензента — это явный операционный риск. Но его возможно преодолеть составивь из связей рецензентов граф и по нему выбирать дублирующих рецензентов. Более подробно [тут](https://github.com/denisbolshakoff/to_analyze_the_connectivity_of_reviewers).

#### Q. Соответствие требованиям РИНЦ/ВАК/Scopus? Не нарушает ли система каких-то требований этих систем?
A: Процедура не нарушается, так как рукопись проходит только через фильтр других рукописей и не загружается ни в какие другие базы данных.

#### Q. Как использовать парафраз?
A: Парафраз может использоваться дополнительно к типовом письме рецензенту с просьбой принять статью к рецензированию. По опыту замечено, что рецензент не сразу после открытия письма начинает работать с рукописью. Этот лаг между получением письма и началом работы с рукописью может быть уменьшен, если тематика рукописи рецензенту не близка и он отказывается уже на этапе получения электронного письма. В парафразе содержится краткое изложение всей статьи взятое из всего текста, а не только из аннотации и позволяет рецензенту в двух трех предложения оценить подходит ли ему темаматика или нет.

#### A. Что означает число (вероятность) для редактора рукописи?
A: Вероятность означает близость вновь пришедшей рукописи к тем рукописям, что были рассмотрены редакционной коллегией журнала ранее. Близость к числу 1 означает, что рукопись скореe всего будет опубликована. Близоксть к числу 0 означает, что вновь пришедшая рукопись близка к тому, что журнал не публикует и вероятнее всего будет отклонена. Числа, например, близкие к 0,5 могут означать, что для рукорписи нужно принимать решение редактору: отправить её на рецензирование одному эксперту или тому количеству, что принято в журнале. Ошибки модели возможны и зависят от количество собранных рукописей (1 из 10 для 1000 рукописей, 1 из 20 для 2500 рукописей и 1 из 100 для 5000 рукописей)
