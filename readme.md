...
Собираются тексты из научного журнала: неопубликованные с меткой 0 и опубликованные с меткой 1. Далее все тексты отдаются на разбор нейросети. Внутренняя структура нейросети распознает путь неопубликованной статьи к 0, а опубликованной к 1 и обучает веса (w и h на картинке ниже) проходить к нужным выходам 0 или 1. Но для новой статьи можно выдавать не решение, а вероятность, то есть число от 0 до 1 принадлежности рукописи к непубликуемым или публикуемым. Решение за редактором.
...
В приведенной выше нейросети 3 входа для данных, 2 нейрона на скрытом слое, один выход и всего 8 весов. В реальной сети примерно 35000 входов для данных 128 нейронов на скрытом слое, один выход и примерно $4 \thinspace 500 \thinspace 000$ обучаемых весов.
...
* определении вероятности опубликования - *BERT*-модель, предобученная на большом корпусе научных текстов на русском языке, – `ai-forever/ruSciBERT` ($\mathbb{E}[accuracy]\approx0.89$ правильно распознается 9 из 10 рукописей);
* определение рецензентов путём поиска рукописи, наиболее близкой к вновь пришедшей по косинусному расстоянию и выводу рецензетов, которые уже видели максимально похожий материал, мультифункциональная модель – `BAAI/bge-m3` ($accuracy\approx0.98$ правильно распознается рецензенты в 49 из 50 рукописей);
...
> - Модель `CountVectorizer` вместо модели `BAAI/bge-m3` для определения рецензента (*TTFT* - 2 секунды). Полный инференс модели `BAAI/bge-m3` на *CPU* достигает 83 часов, а при использовании *G*4 c 90 *GB GPU* - 3 минуты (на ускорителе *T*4 примерно 20 минут);
