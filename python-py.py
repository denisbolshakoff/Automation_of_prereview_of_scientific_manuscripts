...
# -------------------------
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# На GitHub эта модель не загружена. Требуется переобучение на данных конкретного научного журнала!!
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
# На GiiHub данных файл с фамилиями рецензентов не загружен. Собирается на основании рукописей, отправленных на обучение модели model_checkpoint.pth
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
# На GitHub данный файл с предобработанными статьями не загружен. Требуется его сформировать на основании статей, отправленных на обучение модели model_checklpoint.pth
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! 
...
def T5paraphrase(article_input):
    ...
    #partial_summaries = [generate_summary(chunk, max_new_tokens=10) for chunk in chunks]
    #print(chunks)
    print(" ".join(chunks))
    final_summary = generate_summary(" ".join(chunks), max_new_tokens=50)  # очень осторожно варьируем max_new_tokens приводит к загрузкам процессора и очень долгому времени расчёта на ПК без GPU
